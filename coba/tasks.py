...
def time_out_signed():
	...
	timed_out_students = CheckIn.objects.filter(
		is_on_clock=True
		)
	names = list()
	for student in timed_out_students:
		student.auto_time_out=datetime.now()	
...
def notify_timed_out(students:list)->None:
	...
	"""
	messages = tuple((subject,message.format(student.first_name),settings.EMAIL_HOST_USER,[student.email]) for student in students)
...
def weekly_report():
	...
	students = CheckIn.objects.filter(date_created__range=[startdate,enddate],is_on_clock=False,timed_out=False)
